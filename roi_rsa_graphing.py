...
cdf = c.df.groupby(['trial_type','encode','roi','subject']).mean()
cdf = (cdf.loc['CS+'] - cdf.loc['CS-']).reset_index()
cdf['group'] = cdf.subject.apply(lgroup)

pdf = p.df.groupby(['trial_type','encode','roi','subject']).mean()
...
pfc = pfc.set_index(['group','roi','encode'])

encode = ['baseline','conditioning','early_extinction','extinction']
def cscomp(group,df,rois,n_boot=1000):
    ...
    ci = ci.melt(id_vars='roi',var_name='encode',value_name='ci').set_index(['roi','encode']).sort_index()

    dist = pd.DataFrame.from_dict(out['dist'],orient='index').reset_index().rename(columns={'index':'roi'})
    dist = dist.melt(id_vars='roi',var_name='encode',value_name='dist').set_index(['roi','encode'])
    dist = dist.dist.apply(pd.Series).stack().reset_index(-1,drop=True)
    dist = dist.reset_index().rename(columns={0:'dist'})
    ci['point'] = dist.groupby(['roi','encode']).mean()
    ci = ci.reset_index()
    ...
    for i, roi in enumerate(rois):
        ...
        sns.violinplot(data=dist.loc[roi],x='encode',y='dist',
...
# cscomp('control',pfc,['insula'])
# cscomp('ptsd',pfc,['insula'])
...
##############################################################################
cdf = c.df.groupby(['encode','trial_type','roi','subject']).mean()
cdf = (cdf.loc['fear_conditioning'] - cdf.loc['extinction']).reset_index()
cdf['group'] = cdf.subject.apply(lgroup)

pdf = p.df.groupby(['encode','trial_type','roi','subject']).mean()
pdf = (pdf.loc['fear_conditioning'] - pdf.loc['extinction']).reset_index()
pdf['group'] = pdf.subject.apply(lgroup)
...
# phasecomp('control',pfc,['insula'])
# phasecomp('ptsd',pfc,['insula'])
...
#################################################################################
cdf = c.df.groupby(['trial_type','encode','roi','hc_acc','subject']).mean()
cdf = (cdf.loc['CS+'] - cdf.loc['CS-']).reset_index()
cdf['group'] = cdf.subject.apply(lgroup)

pdf = p.df.groupby(['trial_type','encode','roi','hc_acc','subject']).mean()
pdf = (pdf.loc['CS+'] - pdf.loc['CS-']).reset_index()
pdf['group'] = pdf.subject.apply(lgroup)
...
pfc.roi = pfc.roi.apply(lambda x: 'vmPFC' if x == 'mOFC' else x)
pfc.encode = pfc.encode.apply(lambda x: 'conditioning' if x == 'fear_conditioning' else x)
pfc.hc_acc = pfc.hc_acc.apply(lambda x: 'hit' if x == 1 else 'miss')

pfc = pfc.set_index(['group','roi','encode','hc_acc'])
...
def mem_cscomp(group,df,rois,n_boot=1000):
    ...
    ci = ci.melt(id_vars=['roi','encode'],var_name='mem',value_name='ci').set_index(['roi','encode','mem'])
    ...
    dist = dist.reset_index().rename(columns={0:'dist'})
    ci['point'] = dist.groupby(['roi','encode','mem']).mean()
    ci = ci.reset_index()
    ...
    ci = ci.sort_values(by=['roi','encode','mem'])
    ...
    for i, roi in enumerate(rois):
        ...
        sns.violinplot(data=dist.loc[roi],x='encode',y='dist',hue='mem',split=True,
...
# mem_cscomp('control',pfc,['insula'])
# mem_cscomp('ptsd',pfc,['insula'])
...
#################################################################################
cdf = c.df.groupby(['encode','trial_type','roi','hc_acc','subject']).mean()
cdf = (cdf.loc['fear_conditioning'] - cdf.loc['extinction']).reset_index()
cdf['group'] = cdf.subject.apply(lgroup)

pdf = p.df.groupby(['encode','trial_type','roi','hc_acc','subject']).mean()
pdf = (pdf.loc['fear_conditioning'] - pdf.loc['extinction']).reset_index()
pdf['group'] = pdf.subject.apply(lgroup)
...
pfc.roi = pfc.roi.apply(lambda x: 'vmPFC' if x == 'mOFC' else x)
pfc.hc_acc = pfc.hc_acc.apply(lambda x: 'hit' if x == 1 else 'miss')
pfc = pfc.set_index(['group','roi','trial_type','hc_acc'])
...
pfc = pfc.set_index(['roi','group','encode','hc_acc'])
groups = ['control','ptsd']
mems = ['hit','miss']
...
pfc.roi = pfc.roi.apply(lambda x: 'vmPFC' if x == 'mOFC' else x)
pfc.encode = pfc.encode.apply(lambda x: 'conditioning' if x == 'fear_conditioning' else x)
pfc = pfc.set_index(['roi','encode','block','group']).sort_index()
...
phases = ['baseline','conditioning','extinction']
        ...
        for phase in phases:
            ...
            for block in blocks:
                ...
                out['ci'][group][phase][block], out['dist'][group][phase][block] = pg.compute_bootci(df.loc[(phase,block,group),'rsa'],
                                                                ...
                                                                method='cper',decimals=3,seed=42)
    ci = pd.DataFrame.from_dict(out['ci'],orient='index').reset_index().rename(columns={'index':'group'})
    ci = ci.melt(id_vars='group',var_name='encode',value_name='ci').set_index(['group','encode']).sort_index()
    ci = ci.ci.apply(pd.Series).reset_index()
    ci = ci.melt(id_vars=['group','encode'],var_name='block',value_name='ci').set_index(['group','encode','block'])
    
    dist = pd.DataFrame.from_dict(out['dist'],orient='index').reset_index().rename(columns={'index':'group'})
    dist = dist.melt(id_vars='group',var_name='encode',value_name='dist').set_index(['group','encode'])
    dist = dist.dist.apply(pd.Series).reset_index()
    dist = dist.melt(id_vars=['group','encode'],var_name='block',value_name='dist').set_index(['group','encode','block'])
    dist = dist.dist.apply(pd.Series).stack().reset_index(-1,drop=True)

    dist = dist.reset_index().rename(columns={0:'dist'})
    ci['point'] = dist.groupby(['group','encode','block']).mean()
    ci = ci.reset_index()

    ci.group = pd.Categorical(ci.group,groups,ordered=True)
    ci.encode = pd.Categorical(ci.encode,phases,ordered=True)
    ci.block  = pd.Categorical(ci.block,blocks,ordered=True)

    dist.group = pd.Categorical(dist.group,groups,ordered=True)
    dist.encode = pd.Categorical(dist.encode,phases,ordered=True)
    dist.block  = pd.Categorical(dist.block,blocks,ordered=True)

    ci = ci.sort_values(by=['group','encode','block']).reset_index(drop=True)
    dist = dist.sort_values(by=['group','encode','block']).reset_index(drop=True)

    ci = ci.set_index(['group','encode']).sort_index()
