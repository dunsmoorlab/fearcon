...
'''
modified by Gus Hennings (achennings@utexas.edu) to allow for predicitve functions in a sphere
i.e., return non-cross results from train/test splits
useful for training on localizer data and predicting activity in spheres
''' 
...
def _group_iter_search_light(list_rows, estimator, X, y, groups,
    ...
    if type(cv) is int:
        ...
        n_cv = cv.n_splits
    par_scores = np.zeros([len(list_rows),n_cv])
    t0 = time.time()
...
##############################################################################
class Predictive_SearchLight(BaseEstimator):
    ...
    def fit(self, imgs, y, groups=None):
        ...
        if type(self.cv) is int:
            ...
            n_cv = self.cv.n_splits
        scores_3D = {}
        for i in range(n_cv):
            scores_3D[i] = np.zeros(process_mask.shape)
            scores_3D[i][process_mask] = scores[:,i]
