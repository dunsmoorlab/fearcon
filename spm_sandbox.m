...
BIDS_dir = '/Volumes/DunsmoorRed/fmriprep';
ses_dir = fullfile(BIDS_dir,'sub-FC001','ses-1','func');
type (fullfile(ses_dir,'sub-FC001_ses-1_task-acquisition_desc-confounds_regressors.tsv'))
...
if strcmp(args.level, 'item')
    item_names = args.regs{xph}.names;
...
matlabbatch{1}.spm.stats.fmri_spec.timing.units   = 'scans'; %change to seconds
...
matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg   = {multi_regress}; %motion parameters
...
elseif strcmp(args.level, 'item')
    for xitem = 1:n_items
        if args.item_within
            ...
            xweight = zeros(1, n_items);
            xweight(xunit) = -(1/(n_item_within-1));
            xweight(xitem) = 1;
        else
            xweight = ones(1, n_items) * -(1/(n_items-1));
            xweight(xitem) = 1;
        end
        
        matlabbatch{3}.spm.stats.con.consess{xitem}.tcon.name    = ...
            sprintf('%s>others', item_names{xitem});
