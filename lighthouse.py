...
def bin_output():
    for sub in all_sub_args:    
        print(sub)
        subj = meta(sub)
        img = os.path.join(subj.sl_dir,'xval_localizer.nii.gz')
        out = os.path.join(subj.roi,'loc_sl_mask.nii.gz')
        os.system('fslmaths %s -thr .5 -bin %s'%(img,out))

        # out = os.path.join(subj.sl_dir,'loc_mask.nii.gz')
        # new = os.path.join(subj.roi,'loc_sl_mask.nii.gz')
        # os.system('rm %s'%(out))
        img = os.path.join(subj.sl_dir,'predictive_extinction_recall.nii.gz')
        std = '/usr/local/fsl/data/standard/MNI152_T1_1mm_brain.nii.gz'
        out = os.path.join(data_dir,'Group_SL','predictive_rnw','rnw_%s.nii.gz'%(subj.num))
        os.system('flirt -in %s -ref %s -out %s'%(img,std,out))

def group_res():
    bmask = '/usr/local/fsl/data/standard/MNI152_T1_1mm_brain_mask.nii.gz'
    masker = NiftiMasker(mask_img=bmask)
    nvox = 1827243
    control = np.zeros([24,nvox])
    ptsd = np.zeros([24,nvox])

    for i, sub in enumerate(sub_args):
        img = nib.load(os.path.join(data_dir,'Group_SL','predictive_rnw','rnw_%s.nii.gz'%(sub)))
        img = masker.fit_transform(img)[0]
        control[i,:] = img
    
    control_avg = control.mean(axis=0)
    control_avg = masker.inverse_transform(control_avg)
    nib.save(control_avg,os.path.join(data_dir,'Group_SL','predictive_rnw','control_avg.nii.gz'))   
    ...
    ptsd_avg = ptsd.mean(axis=0)
    ptsd_avg = masker.inverse_transform(ptsd_avg)
    nib.save(ptsd_avg,os.path.join(data_dir,'Group_SL','predictive_rnw','ptsd_avg.nii.gz')) 

    comp = np.zeros(nvox)
    for i in range(803283,nvox):
        if control[:,i].sum() == 0 and ptsd[:,i].sum() == 0:
            comp[i] = 0
        else:
            comp[i] = 1 - pg.ttest(control[:,i],ptsd[:,i])['p-val'].values[0]
        if i%10000 == 0:
            print(str(100*np.round(i/nvox,4))+'%')
    comp = masker.inverse_transform(comp)
    nib.save(comp,os.path.join(data_dir,'Group_SL','predictive_rnw','group_comp.nii.gz'))
