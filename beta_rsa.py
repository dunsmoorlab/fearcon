...
class rsa():
	def __init__(self,sub=None,save_dict=None,fs=False,weights='copes',trials='CS+',con='csp',mask=None):
	...
	def load_data(self):
		...
		for phase in self.rsa_phases:				
			...
			#clean out CS- trials
			beta_img = np.delete(beta_img, np.where(run_labels == trash)[0],axis=0)
			#save it to the train data
			#BE CAREFUL WITH YOUR COPYS
		...
		for label in self.rsa_labels:
			print(label)
			self.W[label] = self.W_masker.fit_transform(os.path.join(self.sub_dir,'%s_%s.nii.gz'%(label,self.con)))
		...
		for label in self.rsa_split_labels:
			print(label)
			self.split_W[label] = self.W_masker.fit_transform(os.path.join(self.sub_dir,'%s_%s.nii.gz'%(label,self.con)))
		#apply the weights
...
class group_rsa():
	def __init__(self,subs,mask,trials,con,fs=False):
		...
		self.collect_sub(subs=subs,mask=mask,trials=trials,con=con,fs=fs)
		self.sub_stats()

	def collect_sub(self,subs=None,mask=None,trials=None,con=None,fs=False):	
		...
		for i,sub in enumerate(subs):
			...
			sub_rsa = rsa(sub=sub,mask=mask,trials=trials,con=con,fs=fs)
...
def rsa_graph(mask,trials,con):
	c = group_rsa(sub_args,mask,trials,con);p = group_rsa(p_sub_args,mask,trials,con)
	...
	df.to_csv(os.path.join(data_dir,'graphing','beta_rsa','%s_%s.csv'%(mask,con)),index=False)
	...
	df2.to_csv(os.path.join(data_dir,'graphing','beta_rsa','split_%s_%s.csv'%(mask,con)),index=False)
