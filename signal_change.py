...
def collect_psc():
	...
	for sub in all_sub_args:
		...
		for roi in ['amygdala_beta','mOFC_beta','vmPFC_beta','hippocampus_beta','dACC_beta']:
		#			peg[sub][roi] = {}
 		# for roi in ['vmPFC']:
	...
	peg.to_csv(os.path.join(data_dir,'graphing','signal_change','beta_values.csv'),sep=',',index=False)
...
def collect_ev(imgs='tr',nvox='all',SC=True,S_DS=True,rmv_scram=True,rmv_ind=False,verbose=False,
				split=False,con='CS+',binarize=False,save_dict=ppa_prepped,res=None,pres=None):
	...
	for sub in sub_args:
		cev[sub] = res.exp_ev_df['evidence'].loc[idx[sub,'scene',:,0:end_tr]].groupby(['trial']).mean().mean()
	...
	for sub in p_sub_args:
		p_cev[sub] = pres.exp_ev_df['evidence'].loc[idx[sub,'scene',:,0:end_tr]].groupby(['trial']).mean().mean()
...
def run_featquery(sub=None,roi=None):
	print(sub)
	subj = meta(sub)
