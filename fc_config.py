...
def rm_out(x):
    """Takes in array x, ,removes outliers and returns an array.
    Outliers are defined by data that are beyond
    1.5*Inter Quartile Range in either direction.

    If x is a list of arrays, assumes paired data.
    For list of arrays x = [q,p],
    if q[12] is an outlier, also remove the observation at p[12] 
    """
    RM = np.array([])
    if type(x) is list:
        N = len(x)
        ...
        for j, w in enumerate(x):
            ...
            IQR[j] = iqr(w)
            low_cut[j] = np.percentile(w,25) - 1.5*IQR[j]
            high_cut[j] = np.percentile(w,75) + 1.5*IQR[j]

            for i in w:
                if i < low_cut[j] or i > high_cut[j]:
                    RM = np.append(RM,np.asarray(w == i).nonzero()[0])
        
        #if a subject is an outlier in more than one observation just need 1
        RM = np.unique(RM)
        
        y = [np.delete(w,RM) for w in x]
    else:
        ...
        for i in x:
            if i < low_cut or i > high_cut:
                RM = np.append(RM,np.asarray(x == i).nonzero()[0])
        
        y = np.delete(x,RM)
    
    if len(RM.nonzero()[0]) > 0:
        print('Removing outliers at %s'%(RM))
    else:
...
def barstrip(df,vals,cond=False,points='swarm',ax=False):
    ...
    # def no_point(**kwargs): pass
    
    if type(vals) is str:
        vals = [vals]
    
    #kind of point plot to draw
    if points is 'swarm': pplot = sns.swarmplot
    if points is 'strip': pplot = sns.stripplot 
    
    #mean and SEM for group or group + condition
    if cond:
        _sems = df.groupby(['group','condition']).sem()
        _avgs = df.groupby(['group','condition']).mean()
    else:
        _sems = df.groupby(['group']).sem()
        _avgs = df.groupby(['group']).mean()
    
    #for each value passed
    for val in vals:
        if val not in ['group','subject','condition']:

            if not ax or len(vals) > 1: fig, ax = plt.subplots()
            

            #barplot
            if cond: sns.barplot(x='group',y=val,hue='condition',data=df,palette=cpal,ax=ax,ci=None)
            else:    sns.barplot(x='group',y=val,data=df,palette=gpal,ax=ax,ci=None)

            #points
            if points is not None:
                if cond: pplot(x='group',y='z',hue='condition',data=df,ax=ax,palette=cpoint,dodge=True,
                                       linewidth=2,edgecolor='black',size=8)
                else:    pplot(x='group',y=val,data=df,palette=gpal,ax=ax,
                                       linewidth=2,edgecolor='black',size=8)
            #errorbars
            for i, group in enumerate(df.group.unique()):
                x = ax.get_xticks()[i]
                if points is None: offset = 0
                else: offset = .25
                if cond:
                    ax.errorbar(x-.3,_avgs[val][group]['CS+'],yerr=_sems[val][group]['CS+'],color='black',linewidth=3,capsize=5,capthick=3)
                    ax.errorbar(x+.1,_avgs[val][group]['CS-'],yerr=_sems[val][group]['CS-'],color='black',linewidth=3,capsize=5,capthick=3)
                else:
                    ax.errorbar(x-offset,_avgs[val][group],yerr=_sems[val][group],color='black',linewidth=3,capsize=5,capthick=3)

            sns.despine(ax=ax);
            try: ax.legend_.remove()
            except AttributeError: pass
            if not cond:
                t, p = np.round(pg.ttest(df[val][df.group=='control'],df[val][df.group=='ptsd'])[['T','p-val']].loc['T-test'],4)
                ax.set_title('t = %s; p = %s'%(t,p))
    # plt.tight_layout()
...
fsl_betas_wb = {
    'baseline': 'day1/run001/fsl_betas/run001_beta.npz',
    'fear_conditioning': 'day1/run002/fsl_betas/run002_beta.npz',
    'extinction': 'day1/run003/fsl_betas/run003_beta.npz',
    'extinction_recall': 'day2/run004/fsl_betas/run004_beta.npz' ,
    'memory_run_1': 'day2/run005/fsl_betas/run005_beta.npz',
    'memory_run_2': 'day2/run006/fsl_betas/run006_beta.npz',
    'memory_run_3': 'day2/run007/fsl_betas/run007_beta.npz',
    'localizer_1': 'day2/run008/fsl_betas/run008_beta.npz',
    'localizer_2': 'day2/run009/fsl_betas/run009_beta.npz',
}
...
hipp_no_ppa_beta = {
    'baseline': 'day1/run001/hipp_no_ppa_run001.npz',
    'fear_conditioning': 'day1/run002/hipp_no_ppa_run002.npz',
    'extinction': 'day1/run003/hipp_no_ppa_run003.npz',
    'extinction_recall': 'day2/run004/hipp_no_ppa_run004.npz' ,
    'memory_run_1': 'day2/run005/hipp_no_ppa_run005.npz',
    'memory_run_2': 'day2/run006/hipp_no_ppa_run006.npz',
    'memory_run_3': 'day2/run007/hipp_no_ppa_run007.npz',
    'localizer_1': 'day2/run008/hipp_no_ppa_run008.npz',
    'localizer_2': 'day2/run009/hipp_no_ppa_run009.npz',
}
...
beta_nz_ppa_prepped = {
    'baseline': 'day1/run001/fsl_betas/bnz_ppa_run001.npz',
    'fear_conditioning': 'day1/run002/fsl_betas/bnz_ppa_run002.npz',
    'extinction': 'day1/run003/fsl_betas/bnz_ppa_run003.npz',
    'extinction_recall': 'day2/run004/fsl_betas/bnz_ppa_run004.npz' ,
    'memory_run_1': 'day2/run005/fsl_betas/bnz_ppa_run005.npz',
    'memory_run_2': 'day2/run006/fsl_betas/bnz_ppa_run006.npz',
    'memory_run_3': 'day2/run007/fsl_betas/bnz_ppa_run007.npz',
    'localizer_1': 'day2/run008/fsl_betas/bnz_ppa_run008.npz',
    'localizer_2': 'day2/run009/fsl_betas/bnz_ppa_run009.npz',
}
